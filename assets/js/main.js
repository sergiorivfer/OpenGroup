...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        scrollTopPercentage();
                ...
                // ===========================
        // Modal de contacto OG
        // ===========================
        (function () {
            var $modal = $("#og-contact-modal");
            if (!$modal.length) return;

            function openModal(e) {
                if (e) e.preventDefault();
                $modal.addClass("is-open");
                $("body").addClass("og-contact-modal-open");
            }

            function closeModal(e) {
                if (e) e.preventDefault();
                $modal.removeClass("is-open");
                $("body").removeClass("og-contact-modal-open");
            }

            // Abrir modal
            $(document).on("click", ".js-open-contact-modal", openModal);

            // Cerrar modal (overlay + botón X)
            $(document).on("click", ".js-og-contact-close", closeModal);
    ...
    // [DESHABILITADO] ScrollSmoother eliminado para scroll nativo — refactor menu 2026
   ...
   // Tap móvil para abrir/cerrar tooltips
document.querySelectorAll('.about-section .hs-pin').forEach(btn=>{
    btn.addEventListener('click', (e)=>{
        const tip = e.currentTarget.parentElement.querySelector('.hs-tip');
        const isOpen = tip.dataset.open === '1';
        document.querySelectorAll('.about-section .hs-tip[data-open="1"]').forEach(t=>t.dataset.open='0');
        tip.dataset.open = isOpen ? '0' : '1';
    });
    });
    document.addEventListener('click', (e)=>{
    if(!e.target.closest('.about-section .hs')){
        document.querySelectorAll('.about-section .hs-tip[data-open="1"]').forEach(t=>t.dataset.open='0');
    }
    });

    // ===========================
// Testimonios OG (custom)
// ===========================
document.addEventListener('DOMContentLoaded', function () {
    var section = document.querySelector('.og-testimonials');
    if (!section) return;

    var testimonials = Array.prototype.slice.call(
        section.querySelectorAll('.og-testimonial')
    );
    var thumbs = Array.prototype.slice.call(
        section.querySelectorAll('.og-testimonials__thumb')
    );

    if (!testimonials.length || !thumbs.length) return;

    var currentIndex = 0;
    var autoplayInterval = null;
    var AUTOPLAY_DELAY = 5000; // 5 segundos
    ...
    function showNext() {
        ...
        showTestimonialByIndex(nextIndex);
    }
    ...
    // Estado inicial
    showTestimonialByIndex(0);
    startAutoplay();
});

    // Modal en todas las paginas

    document.addEventListener("click", function (e) {
    const openBtn = e.target.closest(".js-og-contact-open");
    const closeBtn = e.target.closest(".js-og-contact-close");
    const modal = document.getElementById("og-contact-modal");
